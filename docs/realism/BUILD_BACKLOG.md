...
| R1 — hero bird | Technical art + animation | Rebuilt continuous model, feather hierarchy, small rig, three LODs, material set, perch-contact animation and fallback switch. **Planned in detail in [BIRD_PLAN.md](BIRD_PLAN.md)** against the measured bird (44 draw calls of 68; 11,780 tris at ~140 px): merge/decimate first, then procedural v2, then authored surfaces; the rigged-GLB route is a go/no-go after that, gated by a rig-contract test | 5–10 days as written; see BIRD_PLAN for the phased estimate | R0 observation contract. Six-angle inspection plus chase/bank/landing clips; silhouette and contacts pass |
