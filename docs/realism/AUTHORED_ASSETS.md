...
| Decoded budget | **24 MB total** for the whole authored set, RGBA8 with mips. A 512² map is 1.33 MB; a 1024² map is 5.33 MB. That is the real constraint — the download is not. |

**Packed maps.** `_orm` means occlusion in R, roughness in G, metalness in B, which saves two texture units. It is accepted, and the gate checks the one thing that goes wrong: an agent returning a greyscale image and calling it ORM, which silently loses two of the three maps.
...
| Aspect | sky | not 2:1 |
| Decoded size with mips | the whole set | total > 24 MB |
