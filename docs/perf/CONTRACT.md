...
`not-implemented` · `no-extension` · `no-context` · `not-webgl2` · `disjoint` ·
`insufficient-samples` · `paused` · `not-applicable` · `stale`

> **Amendment (G2e, Wave 2R).** `not-webgl2` was added. `src/game/gpu-timer.js` needs to
> separate "this is a WebGL1 context, the extension cannot exist here" from "WebGL2 is
> present and the extension is absent" — the first is a platform fact about the context,
> the second a platform fact about the driver, and collapsing them into `no-extension`
> loses the distinction the module was commissioned to make. The behaviour was right and
> the enum was short a row.
...
| **DEF-3** | **The 30 FPS target-rate row**, with real render pacing | `docs/CUTTING_EDGE_2026.md`: iOS caps rAF at 60 Hz. Half-rate skipping on a 60 Hz panel produces judder that scores **worse** on the very interval statistics this plan adopts (p95, >25 ms count). Shipping it as a menu entry that makes the measured tail worse is an anti-feature. | A **named pacing mechanism** plus device evidence that a 30 FPS profile improves p95/p99 and sustained thermals against the 60 FPS profile on the same route. Until then the panel's Target rate control offers **60 only**. **Amendment (G2e):** Wave 2 shipped it as a single-option row, disabled, stating the deferral, and G2b flagged that against the original wording ('absent — not present-and-disabled'). The amendment keeps the shipped form. The original concern was a menu entry that makes the measured tail worse; a disabled row with one option cannot do that, and the target rate is the `B` in every budget and acceptance figure on the panel, so hiding it costs the reader a number they need to interpret everything else. What remains forbidden is an ENABLED 30/90/120 entry without a named pacing mechanism. | Nothing. The panel's Target rate control ships 60 only. 90/120 likewise deferred (plan: *"only after observed browser cadence and device testing justify them"*). |
