...
> **Amended 2026-09-05 (Mentis).** The product gained a **New Project** screen offering **Clay** (unrigged, subdivided cube, exports a static mesh) or **Humanoid** (rigged, exports an avatar), and **pinned reference cards** were restored to scope after the recut deferred them as "floating boards". Everything below still holds — the export research, the rig rules and the format decision are all about the Humanoid path, which is unchanged — but read it knowing Clay exists and needs no rig, no VRM humanBones map and no `AnalyzeIK` compliance. See §14 of the PRD for the amendment and its cost.
