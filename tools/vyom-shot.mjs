#!/usr/bin/env node
/**
 * vyom-shot.mjs — the screenshot harness.
 *
 * Every visual claim about VYOM gets verified against a real captured frame,
 ...
 *
 * Usage:
 *   node tools/vyom-shot.mjs --page vyom/dev/planet.html --out shots/planet.png
 *   node tools/vyom-shot.mjs --page vyom/index.html --out shots/race.png \
 *        --w 390 --h 844 --dpr 3 --wait 4000 \
 *        --eval "window.__vyom.debugJumpTo(0.42)" --settle 800
 *
 * Flags:
 ...
 *   --wait    ms to wait for window.__VYOM_READY             (default 15000)
...
function findChromium() {
    if (process.env.VYOM_CHROMIUM && fs.existsSync(process.env.VYOM_CHROMIUM)) {
        return process.env.VYOM_CHROMIUM;
    }
...
async function main() {
    ...
    if (!args.page || !args.out) {
        console.error('usage: vyom-shot.mjs --page <html> --out <png> [--w 390 --h 844 --dpr 3]');
        process.exit(2);
    }
    ...
    try {
        ...
        await page.waitForFunction('window.__VYOM_READY === true', null, { timeout: readyTimeout });
    } catch (err) {
        readyOk = false;
    ...
    const stats = await page.evaluate(() => (window.__VYOM_STATS ? window.__VYOM_STATS() : null)).catch(() => null);
