#!/usr/bin/env node
/**
 * gauntlet-shot.mjs — the screenshot harness.
 *
 * Every visual claim about Birb Gauntlet gets verified against a real captured frame,
 ...
 *
 * Usage:
 *   node tools/gauntlet-shot.mjs --page gauntlet/dev/planet.html --out shots/planet.png
 ...
 *   --wait    ms to wait for window.__GAUNTLET_READY             (default 15000)
...
async function main() {
    ...
    if (!args.page || !args.out) {
        console.error('usage: gauntlet-shot.mjs --page <html> --out <png> [--w 390 --h 844 --dpr 3]');
        process.exit(2);
    }
    ...
    try {
        ...
        await page.waitForFunction('window.__GAUNTLET_READY === true', null, { timeout: readyTimeout });
    } catch (err) {
        readyOk = false;
    ...
    const stats = await page.evaluate(() => (window.__GAUNTLET_STATS ? window.__GAUNTLET_STATS() : null)).catch(() => null);
