...
**Content expansion + nest-clumping fix (2026-06-02):** forest read well but canyon/mountain/city felt thin and their nests ("beats") clumped. **Nest root cause:** nests were emitted only from the *tightly-packed cluster props* (canyon ridges / mountain ranges / city blocks) via modulo counters, so 2-5 nests knotted inside one cluster while the evenly-distributed **scatter layer** (the props the bird actually flies through) got none — knots on a few distant clusters, nothing nearby. **Fix:** a shared `tryAddNest(nestablePositions, pos, normal, NEST_MIN_SPACING)` gate (rejects a nest within `NEST_MIN_SPACING` = 34 world-u of an existing one; O(n²) over the small list, build-time only) now wraps EVERY nest emission in canyon/mountain/city, PLUS new scatter-prop nest passes (every Nth tall scatter spire / valley pine / scatter tower) so nests spread evenly and sit on real props — incl. mountain nests now riding actual scatter pines ("nests on the trees"). **Forest nest logic is intentionally untouched** (it was already good — well-spread groves + emergent nest trees). **Content:** canyon gains a **mesa/plateau** InstancedMesh (flat-topped buttes, low skimmable + tall landmark, flat tops host nests) — its missing signature landform; mountain gains a **rock-crag** InstancedMesh (thin grey spires anchored near peaks for verticality) + denser ranges (7/9→8/11 ranges, more peaks/range, snow threshold 35→30, pine groves 6/8→7/10); city gains **cyberpunk neon** — per-instance-colored (`setColorAt`/instanceColor → still 1 draw call each) neon **fins** on tower faces, **antenna beacon tips** (red/cyan), brighter denser **neon traffic** (hover rings unlit MeshBasic, 14/38→20/52), + more blocks (11/13→13/16) and scatter sprawl (150/250→180/300). All new props are InstancedMesh (≈1 draw call each, biomes still well under the <100 budget) with build-time-only allocation; the ≤0 terrain-floor invariant is NOT touched.
