export function createFollowCameraRig(three, options = {}) {
  ...
  const state = {
    camera: null,
    initialized: false,
    offset: options.offset ? options.offset.clone() : new Vector3(0, 1.05, 2.8),
    position: new Vector3(),
    lookAt: new Vector3(),
    desiredPosition: new Vector3(),
    desiredLookAt: new Vector3(),
    positionDamping: options.positionDamping ?? 0.18,
    lookAtDamping: options.lookAtDamping ?? 0.22,
    rotationDamping: options.rotationDamping ?? 0.18,
    velocityLookAhead: options.velocityLookAhead ?? 0.58,
    steeringLookAhead: {
      forward: options.steeringLookAhead?.forward ?? 0.85,
      strafe: options.steeringLookAhead?.strafe ?? 0.6,
      lift: options.steeringLookAhead?.lift ?? 0.42,
    },
