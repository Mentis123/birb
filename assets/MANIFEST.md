...
|---|---|---|---|---|---|---|
| `textures/bark_pine_albedo.png` | sRGB albedo | `barkMat.map` + `trunkMat.map` (landmark trunk, fallen log, and every instanced forest trunk) — **shipping default**, `?bark=0` to disable | ~4.3 m square | OpenAI image generation; prompt "seamless, unlit grey-brown weathered pine bark plates with deep vertical fissures"; from scratch, resized to 512² RGB with Pillow 12.3.0 | ISC | `e20a04b` |
