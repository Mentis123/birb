...
export class FreeFlightController {
  ...
  update(deltaTime = 0) {
    ...
    // Lift is reduced when diving (moving downward) and the wings are pointed down
    ...
    if (speed > MIN_FLIGHT_SPEED) {
      ...
      // Lift is generated based on how much the wings are facing upward (positive Y component)
      // When wings are level or facing up, generate full lift
      // When wings are facing down (diving), reduce lift proportionally
      const wingUpAmount = Math.max(0, wingUp.y);

      // Also reduce lift based on downward velocity (actual diving motion)
      const verticalVelocityFactor = this.velocity.y < 0
        ? Math.max(0, 1 - Math.abs(this.velocity.y) / MAX_SAFE_SPEED)
        : 1.0;

      const liftEfficiency = wingUpAmount * verticalVelocityFactor;
