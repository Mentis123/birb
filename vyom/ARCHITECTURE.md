# VYOM — architecture contract

> **VYOM** (व्योम, "sky") is a stylised arcade bird-racing game on a miniature
> planet. It lives at `/vyom` on the Birb Mobile site as an unlisted Birb Labs
> artefact. This document is the contract every module builds against — read it
> before writing a line.

## Hard rules

1. **Self-contained.** `vyom/` imports nothing from the parent Birb Mobile
   `src/`. Nothing outside `vyom/` may import from inside it. Birb Mobile must
...
3. **No build step.** Vanilla ES modules, Three.js from a pinned CDN via
   ...
   (that includes nipplejs — VYOM has its own joystick).
4. **Absolute paths.** All same-origin URLs are written `/vyom/...`. Relative
   paths break because Vercel serves this page at `/vyom` *without* a trailing
...
tools/vyom-shot.mjs           screenshot harness (done)
tests/vyom-*.test.js          pure-logic unit tests (node --test, no deps)
```
...
```
Must be unit-tested in `tests/vyom-race-logic.test.js` with `node --test` and no
...
1. Build a probe page at `vyom/dev/<yourname>.html` that renders your subsystem
   in isolation, sets `window.__VYOM_READY = true` after the first render, and
   exposes `window.__VYOM_STATS = () => ({ drawCalls, triangles })`.
2. Capture it:
   ```bash
   node tools/vyom-shot.mjs --page vyom/dev/<yourname>.html \
