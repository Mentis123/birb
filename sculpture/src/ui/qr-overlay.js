 ...
 *
 * Ported verbatim from Birb Gauntlet apart from a configurable kicker. The two
 * artefacts stay airtight against each other, so this is a copy on purpose —
 * a shared module across the site would couple two independent asset graphs.
 *
...
const STYLE_ID = 'sculpture-qr-style';
...
.sq-close:active { transform: translateY(4px); box-shadow: 0 1px 0 0 #0f1c33; }
@media (prefers-reduced-motion: reduce) {
  .sq-wrap, .sq-card { animation: none !important; }
}
`;
...
export function createQrOverlay(opts = {}) {
    ...
    kicker.className = 'sq-kicker';
    kicker.textContent = opts.kicker || 'Scan to view';
